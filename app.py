...
# --- ID CONFIGURATION ---
# NIFTY Spot (For Price History)
SPOT_ID = "13"          
SPOT_SEGMENT = "IDX_I"  

# NIFTY FNO (For Option Chain)
# Note: For Indices, Dhan often uses the SAME ID (13) but usually 'NSE_FNO' segment.
# If this fails, we will auto-detect inside the function.
FNO_SEGMENT = "NSE_FNO" 
...
def get_expiry_date():
    """
    Robust Expiry Fetcher. 
    Tries multiple segments to find where the Expiry list is hiding.
    """
    try:
        # Attempt 1: Standard NSE_FNO
        ...
        if resp['status'] == 'success' and resp['data']:
            ...
            return sorted([d for d in dates if str(d).count('-')==2])[0]
            
        # Attempt 2: IDX_I (Rare, but sometimes works for Spot-linked expiries)
        resp = dhan.expiry_list(
            under_security_id=int(SPOT_ID), 
...
def get_market_analysis():
    ...
    if not expiry:
        st.error("❌ Could not find Expiry Date in any segment.")
        return None
        
    try:
        oc_resp = dhan.option_chain(
            under_security_id=int(SPOT_ID), 
            under_exchange_segment="NSE_FNO", # Option Chain is ALWAYS FNO
            expiry=expiry
        )
