...
def get_market_analysis():
    ...
    gamma_msg = ""
    if abs(ltp - res) < 15: gamma_msg = f"⚠️ Near Call Wall ({res}). Possible Blast!"
    if abs(ltp - sup) < 15: gamma_msg = f"⚠️ Near Put Wall ({sup}). Possible Blast!"
...
# 4. UI
# ---------------------------------------------------------
st.title("🎯 Nifty Gamma Hunter (Auto-Refreshes)")
...
if data:
    ...
    c3.metric("Resistance", data['res'])
    c4.metric("Support", data['sup'])
...
# ---------------------------------------------------------
# 5. AUTO REFRESH LOGIC (VISUAL)
# ---------------------------------------------------------
st.divider()
st.write("⏳ Next update in:")
